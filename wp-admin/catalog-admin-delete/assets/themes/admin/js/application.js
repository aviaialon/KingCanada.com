    ...
    SYSTEM.APPLICATION.MAIN.prototype.bytesToSize = function(bytes)
    {
     ...
     SYSTEM.APPLICATION.MAIN.prototype.onAddNewManual = function(manual)
     {
		...
		*/
		...
		ulList.find('.noManuals').removeClass('hide');
     ...
     SYSTEM.APPLICATION.MAIN.prototype.addCategorySelectionRow = function(isInit)
     {
        ...
        $(".multiselect-container input").uniform({radioClass: 'choice', selectAutoWidth: false});
         ...
         // Multi select single
         /*
         $('.mainCategorySelector').multiselect({
            ...
            onDropdownShow:  function(event) {
                $('li.disabled').on(SYSTEM.APPLICATION.MAIN.STATUS.EVENT.CLICK, function (event) {
                    event.preventDefault();
                    return false;
                });
            },
            onChange: function(option, checked) {
                ...
                //console.log(me.getVariable(SYSTEM.APPLICATION.MAIN.CONFIGURATION.CATEGORY_TREE));

                // Get selected options.
                var target = this.options.associatedElement,
                    selectedOptions = target.find('option:selected'),
                    dropdown = this.$ul;
                ...
                if (selectedOptions.length >= 1) {
                    ...
                    var nonSelectedOptions = $('option', target).filter(function() {
                    ...
                    $('option', target).each(function() {
                        var input = $('input[value="' + target.val() + '"]');
                        input.prop('disabled', false);
                        input.parents('li').removeClass('disabled');
                    });

                    $('li', dropdown).each(function(index, element) {
                        $(element).find('input[type="checkbox"]').removeProp('disabled');
         ...
         switch (strModuleName)
         {
                    ...
                    var table = $('.datatable-add-row table').DataTable({
                        ...
                        tableTools: {
                            ...
                            aButtons: [{
                                ...
                                /*{
                                    sExtends:    'div',
                                    sButtonText: '<button class="btn btn-primary dropdown-toggle" data-toggle="dropdown">' +
                                                '    <i class="icon-command"></i> Bulk Action  <span class="caret"></span>' +
                                                '</button>' +
                                                '<ul class="dropdown-menu icons-right">' +
                                                '    <li><a href="#"><i class="icon-cogs"></i> Enable</a></li>' +
                                                '    <li><a href="#"><i class="icon-cogs"></i> Disable</a></li>' +
                                                '</ul>',
                                    sButtonClass: ''
                                },
                                */
                                /*{
                                    sExtends:    'div',
                                    sButtonText: '<button class="btn btn-primary dropdown-toggle" data-toggle="dropdown">' +
                '    <i class="icon-command"></i> Action  <span class="caret"></span>' +
                '</button>' +
                '<ul class="dropdown-menu icons-right">' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'json\',escape:\'false\'});"> <img src=\'icons/json.png\' width=\'24px\'> JSON</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'json\',escape:\'false\',ignoreColumn:\'[2,3]\'});"> ' +
                '<img src=\'icons/json.png\' width=\'24px\'> JSON (ignoreColumn)</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'json\',escape:\'true\'});"> <img src=\'icons/json.png\' width=\'24px\'> JSON (with Escape)</a></li>' +
                '    <li class="divider"></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'xml\',escape:\'false\'});"> <img src=\'icons/xml.png\' width=\'24px\'> XML</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'sql\'});"> <img src=\'icons/sql.png\' width=\'24px\'> SQL</a></li>' +
                '    <li class="divider"></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'csv\',escape:\'false\'});"> <img src=\'icons/csv.png\' width=\'24px\'> CSV</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'txt\',escape:\'false\'});"> <img src=\'icons/txt.png\' width=\'24px\'> TXT</a></li>' +
                '    <li class="divider"></li>                ' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'excel\',escape:\'false\'});"> <img src=\'icons/xls.png\' width=\'24px\'> XLS</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'doc\',escape:\'false\'});"> <img src=\'icons/word.png\' width=\'24px\'> Word</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'powerpoint\',escape:\'false\'});"> <img src=\'icons/ppt.png\' width=\'24px\'> PowerPoint</a></li>' +
                '    <li class="divider"></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'png\',escape:\'false\'});"> <img src=\'icons/png.png\' width=\'24px\'> PNG</a></li>' +
                '    <li><a href="#" onClick ="$(\'#tblReportProducts\').tableExport({type:\'pdf\',escape:\'false\'});"> <img src=\'icons/pdf.png\' width=\'24px\'> PDF</a></li>' +
                '</ul>',
                    ...
                    $("#category-selector").on("change", function (e) {
                        ...
                        if (($('#categories').val() == '') || !$('#categories').tagExist(e.val)) {
                            $('#categories').addTag(e.val);
                        }
                    });
                    */
            ...
            case (SYSTEM.APPLICATION.MAIN.MODULE.PRODUCT_EDITOR) :
            {
				...
				$(".filter-manuals").select2({
					...
					var container = $(this).parents('.manuals').find('.manual-list');
					switch (Number(e.val)) {
                 ...
                 * Bulk action events
                 */
                $('a.bulk_action').on(SYSTEM.APPLICATION.MAIN.STATUS.EVENT.CLICK, function(event) {
                    event.preventDefault();
                ...
                $('.deleteManual').on(SYSTEM.APPLICATION.MAIN.STATUS.EVENT.CLICK, document,  function(e) {
                    e.preventDefault();
                    $('#confirm_deleteManual_modal').modal({backdrop: false});
                    ...
                    init : {
                        ...
                        FileUploaded: function(up, file, info) {
                            ...
                            if (response.success == true) {
								...
								$('#uploadedManualSize').val(file.size / 1024); // size in KB
								$('#uploadedManualExtension').val(file.fileExt); // size in KB
							}
                        }
                    },
                    rename: true,
            ...
            case (SYSTEM.APPLICATION.MAIN.MODULE.PRODUCT_EDITOR_EVENTS) :
            {
                ...
                /*
                $("#e15").select2({tags:["red", "green", "blue", "orange", "white", "black", "purple", "cyan", "teal"], maximumSelectionSize: 1});
                ...
                */
                ...
                $('.addAttribute').on(SYSTEM.APPLICATION.MAIN.STATUS.EVENT.CLICK, function(event) {
                    ...
                    var attrCount     = {};
                        attrCount.en  = $('.attributes.en .attribute').length,
                        attrCount.fr  = $('.attributes.fr .attribute').length,
                    lang              = $(this).attr('rel-lang'),
                    attributeTemplate = '<div class="attribute">'
                                            + '<div class="col-md-6">'
                                            + ' <i class="icon-menu2 drgAttribute pull-left"></i>'
                                            + '    <input type="text" name="attributes[' + lang + '][' + attrCount[lang] + '][name]" class="product_attribute pull-right" '
                    ...
                    $(['en', 'fr']).each(function(index, lang) {
                        ...
                        $('.' + __lang + ' .product_attribute').select2({
                            allowClear: true,
                            data: ___data[__lang],
