    ...
    extends \Core\Net\HttpRequest
{
    ...
    */
    protected final function productAction(array $requestDispatchData)
    {
        ...
        $this->setViewData('apiUrl', $this->route('api', 'v:2.03', array()));
    ...
    */
    protected final function save(array $requestDispatchData)
    {
        ...
        if (false === $return['error']) {
            ...
            $product->getDescription('en')->setTitle($productData['title_en']);
            $product->getDescription('en')->setDescription($productData['desc_en'])->save();
            $product->getDescription('fr')->setProductId((int) $product->getId());
            $product->getDescription('fr')->setTitle($productData['title_fr']);
            $product->getDescription('fr')->setDescription($productData['desc_fr'])->save();
